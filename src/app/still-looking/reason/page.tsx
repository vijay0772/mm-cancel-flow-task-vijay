...
export default function StillLookingReasonPage() {
	const router = useRouter();
	const [selected, setSelected] = useState<Reason | null>(null);

	const routes: Record<Reason, string> = {
		"Too expensive": "/still-looking/reason/too-expensive",
		"Platform not helpful": "/still-looking/reason/platform-not-helpful",
		"Not enough relevant jobs": "/still-looking/reason/not-enough-relevant-jobs",
		"Decided not to move": "/still-looking/reason/decided-not-to-move",
		"Other": "/still-looking/reason/other",
	};
				...
				<div className="grid grid-cols-1 md:[grid-template-columns:minmax(0,1fr)_400px] gap-0 md:gap-8 p-6 md:p-8 md:items-stretch">
					...
					<div className="order-2 md:order-1 self-start">
						...
						<div className="mt-2 space-y-3">
							{REASONS.map((r) => (
								<label key={r} className="flex items-center gap-3 cursor-pointer select-none text-[#1f2937]" onClick={() => setSelected(r)}>
									<span className={`w-5 h-5 rounded-full flex items-center justify-center ${selected === r ? "bg-[#41403D]" : "border-2 border-[#41403D]"}`}>
										<span className={`w-2.5 h-2.5 rounded-full ${selected === r ? "bg-white" : "bg-transparent"}`} />
									</span>
									<span>{r}</span>
								</label>
							))}
							...
							<button
								onClick={onComplete}
								disabled={!selected}
								className={`mt-4 w-full h-[44px] rounded-lg font-semibold ${selected ? "bg-[#DC2626] text-white" : "bg-[#E6E6E6] text-[#B5B3AF]"}`}
							>
					...
					<div className="order-1 md:order-2 hidden md:flex md:flex-col md:justify-between md:items-end self-stretch relative">
						...
						<img src="/VN4.png" alt="City image" />
					</div>
					</div>
				</div>
			</div>
