...
export default function PlatformNotHelpfulPage() {
			...
			<div className="relative px-6 py-4 border-b border-gray-200 hidden md:block">
				...
				<div className="grid grid-cols-1 md:[grid-template-columns:minmax(0,1fr)_400px] gap-0 md:gap-8 p-6 md:p-8 md:items-stretch">
					...
					<div className="order-2 md:order-1 self-start">
						...
						<p className="mt-2 text-[14px] text-[#1f2937]">What can we change to make the platform more helpful?*</p>
						<p className="mt-1 text-[13px] text-[#DC2626]">Please enter at least 25 characters so we can understand your feedback*</p>
						...
						<div className="border-t border-gray-200 pt-4 mt-2">
							...
							<button disabled={!valid} onClick={async () => { if (!valid) return; await finalizeCancellation({ reason: feedback.trim(), accepted: false }); router.push("/still-looking/completed"); }} className={`mt-4 w-full h-[44px] rounded-lg font-semibold ${valid ?  "bg-[#DC2626] text-white" : "bg-[#E6E6E6] text-[#B5B3AF]"}`}>
