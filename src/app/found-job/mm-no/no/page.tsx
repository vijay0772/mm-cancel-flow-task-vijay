...
export default function MMNoBranchPage() {
	...
	return (
		...
		<div className="px-4 py-3  md:hidden">
				...
				<div className="grid grid-cols-1 md:[grid-template-columns:minmax(0,1fr)_400px] gap-0 md:gap-8 p-6 md:p-8 md:items-start">
					...
					<h2 className="text-[25px] md:text-[32px] font-semibold text-[#41403D]  leading-none">You landed the job!</h2>
						<p className="mt-0 text-[25px] italic font-bold text-[#41403D] leading-none">That’s what we live for.</p>
						<p className="mt-2 text-[18px] font-bold md:text-[16px] text-[#41403D] leading-tight">
							Even if it wasn’t through Migrate Mate,<br className="hidden md:block" />
							let us help get your visa sorted.
						</p>


						<p className="mt-2 text-[14px] font-bold text-[#62605C]">Is your company providing an immigration lawyer to help with your visa?</p>
							...
							<label className="flex items-center gap-3 cursor-pointer select-none">
								...
								<span className="text-[14px] text-[#41403D] font-medium">No</span>
							</label>
						</div>

						<p className="mt-3 text-[14px] text-[#41403D]">We can connect you with one of our trusted partners.<br/>Which visa would you like to apply for?*</p>
						<input
							className="mt-2 w-full h-[40px] rounded-md border text-gray-900 border-[#62605C] px-3 text-[14px] focus:outline-none focus:ring-2 focus:ring-[#9A6FFF]"
							placeholder=""
							value={visa}
						...
						<div className="border-t border-gray-200 pt-4 mt-2">
							<button
								disabled={!isValid}
								onClick={async () => { if (!isValid) return; await finalizeCancellation({ reason: "No company lawyer - visa: " + visa.trim(), accepted: false }); router.push("/found-job/complete"); }}
					...
					<div className="order-1 md:order-2 hidden md:flex md:justify-end md:items-start">
						...
						<img src="/ON2.png" alt="MM NO image" />
						</div>
					</div>
				</div>
			</div>
